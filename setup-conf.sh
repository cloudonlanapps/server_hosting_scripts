...
MISSING_DECL=()
for v in PRODUCT_CONF_NAME PRODUCT_PROJECT PRODUCT_GIT_URL PRODUCT_PASS_PREFIX; do
    [ -n "${!v:-}" ] || MISSING_DECL+=("$v")
done
...
if [ "$MODE" = upgrade ] && [ "$FORCE_UPGRADE" != 1 ]; then
    ...
    Nothing to configure. The conf already carries every setting this server
    version knows about ($INSTALLED_VERSION -> $TARGET_VERSION is a patch).
...
# ── Which environments live on this host ────────────────────────────────────
# The only genuinely per-machine question. The conf then contains ONLY these,
# so a dev box cannot deploy prod: prod is not in its conf at all.
SELECTED_ENVS="$(declare -p ENVS >/dev/null 2>&1 && printf "%s" "${ENVS[*]}" || printf dev)"
echo
...
# ── Ports ───────────────────────────────────────────────────────────────────
# A product value, because the allocation is kept disjoint across products so
# they cannot collide — but still asked, because this machine may already have
