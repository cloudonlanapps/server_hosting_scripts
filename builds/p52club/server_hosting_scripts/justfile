...
#
# SCRIPTS is passed in rather than derived. justfile_directory() would resolve
# to the importing file's directory under an import, and source_directory()
# — which does the right thing — needs just 1.23, newer than distributions
# ship. The importing justfile already knows where it put these scripts.
...
_require-conf:
    ...
    if [ -z "${SCRIPTS:-}" ] || [ ! -d "${SCRIPTS:-}" ]; then
        echo "ERROR: SCRIPTS is unset or not a directory: '${SCRIPTS:-}'" >&2
        echo "       The importing justfile must export it." >&2
        exit 1
    fi
    if [ -z "${CONF:-}" ] || [ ! -f "${CONF:-}" ]; then
        echo "ERROR: CONF is unset or not a file: '${CONF:-}'" >&2
        echo "       Run from a deployment directory whose justfile imports this one," >&2
        echo "       or set it: SCRIPTS=... CONF=... just -f .../justfile <recipe>" >&2
        exit 1
    fi
